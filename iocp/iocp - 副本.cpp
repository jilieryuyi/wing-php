...
static SOCKET  m_sock_listen;    //the server listen socket
...
private:
	...
	static void onerror( iocp_overlapped *&povl );
...
{ 
	...
	this->m_send_timeout   = send_timeout;               //send timeout
	                                                     //clients 
	this->m_povs           = new unsigned long[max_connect];   
...
void WingIOCP::onerror( iocp_overlapped *&povl ){
	printf("some error happened , error code %d \r\n", WSAGetLastError());
}
//event callbacks
