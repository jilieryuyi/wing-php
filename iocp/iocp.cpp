...
//event callbacks
 void WingIOCP::onconnect( iocp_overlapped *&pOL ){
	...
	// printf("%s %d connect\r\n",inet_ntoa(pOL->m_addrClient.sin_addr), pOL->m_addrClient.sin_port);
	 ...
	 live.keepalivetime		= 500;     //多长时间发送一次心跳包 1分钟是 60000 以此类推     
...
BOOL WingIOCP::start(){	
	...
	do{ 
		...
		for( int i = 0 ; i < this->m_max_connect ; i++ ) 
		{
			...
			//if( 0 != ::setsockopt( client, SOL_SOCKET, SO_REUSEADDR,(LPCSTR)&bReuse, sizeof(BOOL) ) )
			//{
				//some error happened
				
			//}
			//if( 0 != ::setsockopt( client, SOL_SOCKET, 15,(LPCSTR)&bReuse, sizeof(BOOL) ) )
			//{
				//some error happened
